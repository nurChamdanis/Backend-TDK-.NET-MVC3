...
namespace ASPNETMVC3TDK.Models.Achivement
{
    public class AchivementRepo
    {
        #region Singleton

        private static AchivementRepo instance = null;
        IDBContext db = DatabaseManager.Instance.GetContext();
        /*IDBContext db2 = DatabaseManager.Instance.GetContext("SecurityCenter");*/

        public static AchivementRepo Instance
        {
            get
            {
                if (instance == null)
                {
                    instance = new AchivementRepo();
                }
                return instance;
            }
        ...
        public IList<M_Achivement> GetTrainingAchivement(string P_NOREG, string P_ORDER_TYPE, string P_STATUS, bool MODEMASTER, M_Achivement m)
        {
            IList<M_Achivement> result;
            dynamic args = new
            {
                P_NOREG,
                P_ORDER_TYPE,
            ...
            if (MODEMASTER)
            {
                result = db.Fetch<M_Achivement>("Training/Achivement/getMasterAchivement", args);
            }
            else
            {
                result = db.Fetch<M_Achivement>("Training/Achivement/getAchivement", args);
            }
            db.GetSqlLoaders();
            db.Close();
            return result;
        ...
        public async Task<ResultMessage> UpdateData(M_Achivement m)
        {
            ...
            try
            {
                ...
                string contentFolderPath = Path.Combine(personalInformationFolderPath, "Achivement");
                ...
                if (Directory.Exists(contentFolderPath))
                {
                    ...
                    if (m.CERTIFICATE != null && m.CERTIFICATE.FirstOrDefault() != null && contentFolderPath != null)
                    {
                        ...
                        fileNames = $"AC_{m.NOREG}_{timestamp}";
                        ...
                        if (uploadFile)
                        {
                            dynamic args = new
                            {
                                ...
                                P_CERTIFICATE_PATH = "/Content/Document/Training/Achivement/",
                                P_REMARK_1 = m.REMARK_1,
                                P_REMARK_2 = m.REMARK_2
                            };
                            result = db.Fetch<ResultMessage>("Training/Achivement/getUpdateData", args)[0];
                        }
                    }
                    else
                    {
        ...
        public ResultMessage DeleteData(M_Achivement m)
        {
            dynamic args = new
            {
                P_TRANSACTION_ID = m.TRANSACTION_ID
            };
            ResultMessage Result = db.Fetch<ResultMessage>("Training/Achivement/getDeleteData", args)[0];
            db.Close();
            return Result;
        }
