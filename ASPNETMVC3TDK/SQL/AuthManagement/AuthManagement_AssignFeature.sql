...
BEGIN
    BEGIN TRY
        IF ((SELECT COUNT(1) FROM TB_M_AUTHORIZATION WHERE APPLICATION = @@APPALIAS AND USERNAME = @@USERNAME AND ROLE = @@ROLE AND [FUNCTION] = @@FUNCTION AND FEATURE = @@FEATURE) > 0) BEGIN
            SET @@info = 'failed';
            SET @@result = 'false';
