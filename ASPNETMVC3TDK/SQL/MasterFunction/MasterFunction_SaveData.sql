...
BEGIN TRY
		IF(COALESCE(@@MODE, 'ADD') = 'ADD') BEGIN
			...
			IF ((SELECT COUNT(1) FROM TB_M_FUNCTION WHERE APPLICATION = @@APPALIAS AND ID = @@ID) > 0) BEGIN
				SET @@info = 'failed';
				SET @@result = 'false';
				SET @@messages = 'Failed to save, Function ID already exist!';
			END ELSE BEGIN
