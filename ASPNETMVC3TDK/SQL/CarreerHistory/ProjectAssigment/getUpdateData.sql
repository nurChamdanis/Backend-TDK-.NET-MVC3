...
DECLARE @@PROJECT_NAME VARCHAR(MAX) = @P_PROJECT_NAME ;
...
DECLARE @@CODE_TYPE VARCHAR(MAX) = 'PA';

DECLARE @@APPROVAL VARCHAR(MAX);
DECLARE @@COUNT_MATCH VARCHAR(MAX);
...
BEGIN TRY
	BEGIN 
		...
		ELSE
		BEGIN 
			SET @@MAX_TRANSACTION_ID = (SELECT MAX(TRANSACTION_ID) TRANSACTION_ID FROM PDI.TB_R_SUBMISSION_PROJECT_ASSIGNMENT
	...
	--		 CHECK ALL APPROVER 4
			...
			SET @@COUNT_MATCH = (
				SELECT COUNT(*) FROM PDI.TB_M_PROJECT_ASSIGNMENT B 
	...
	IF(@@COUNT_MATCH > 0) 
				BEGIN 
					UPDATE PDI.TB_M_PROJECT_ASSIGNMENT 
					SET 
					CLASS = @@CLASS, POSITION_ABBR = @@POSITION_ABBR ,
				...
				BEGIN
					...
					INSERT INTO PDI.TB_M_PROJECT_ASSIGNMENT
					(
					NOREG , CLASS, POSITION_ABBR, DIVISION_ID, DIVISION_NAME, DEPARTMENT_ID, DEPARTMENT_NAME,
					SECTION_ID, SECTION_NAME, PROJECT_NAME, VALID_FROM, VALID_TO, RESPONSIBILITY, 	
					ACCOMPLISHMENT, REMARK_1, CREATED_DT, CREATED_BY
					)
					VALUES (
					@@NOREG ,
					@@CLASS ,
