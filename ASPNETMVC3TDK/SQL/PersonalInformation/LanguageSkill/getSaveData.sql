	...
	--		 CHECK ALL APPROVER 4
			...
			SET @@COUNT_MATCH = (
					SELECT COUNT(*) FROM PDI.TB_M_PROFILE_LANGUAGE B 
							...
							AND B.LANGUAGE_TEST = @@LANGUAGE_TEST
				);
			...
			BEGIN  
				...
				BEGIN  
					...
					SET @@info = 'failed';
					SET @@result = 500;
					SET @@messages = 'Already data exists !' 

				END ELSE
				BEGIN
					
					IF(@@CERTIFICATE_NAME IS NULL AND @@CERTIFICATE_PATH IS NULL)  BEGIN 
						
						INSERT PDI.TB_M_PROFILE_LANGUAGE 
						(SKILL_LEVEL, SCORE, LANGUAGE_TEST, VALID_TO, REMARK_1, CREATED_DT, CREATED_BY, NOREG, LANGUAGE_ID )
						VALUES 
						(@@SKILL_LEVEL, @@SCORE ,@@LANGUAGE_TEST, @@VALID_TO ,@@REMARK_1, @@CREATED_DT , @@CREATED_BY, @@NOREG, @@LANGUAGE_ID);
					...
					( TRANSACTION_ID, NOREG, LANGUAGE_ID, SKILL_LEVEL, SCORE, LANGUAGE_TEST, CERTIFICATE_NAME, CERTIFICATE_PATH, VALID_TO, STATUS_CD, REJECT_REASON, APP_SH_NOREG, 
					  APP_SH_STATUS, APP_SH_DATE, APP_DPH_NOREG, APP_DPH_STATUS, APP_DPH_DATE, APP_DH_NOREG, APP_DH_STATUS, APP_DH_DATE, APP_DIR_NOREG, 
					  APP_DIR_STATUS, APP_DIR_DATE, APP_HR_ADMIN_NOREG, APP_HR_ADMIN_STATUS, APP_HR_ADMIN_DATE, REMARK_1, REMARK_2, CANCEL_FLAG, CANCEL_BY, 
					  CANCEL_DATE, CREATED_DT, CREATED_BY, UPDATED_DT, UPDATED_BY
					)
					VALUES 
					( @@TRANSACTION_ID, @@NOREG, @@LANGUAGE_ID, @@SKILL_LEVEL, @@SCORE, @@LANGUAGE_TEST, @@CERTIFICATE_NAME, @@CERTIFICATE_PATH, @@VALID_TO, '99', @@REJECT_REASON, @@APP_SH_NOREG,
					  @@APP_SH_STATUS, NULLIF(@@APP_SH_DATE, ''), @@APP_DPH_NOREG, @@APP_DPH_STATUS, NULLIF(@@APP_DPH_DATE, ''), @@APP_DH_NOREG, @@APP_DH_STATUS, NULLIF(@@APP_DH_DATE, ''), @@APP_DIR_NOREG,
					  @@APP_DIR_STATUS, NULLIF(@@APP_DIR_DATE, ''), @@APP_HR_ADMIN_NOREG, @@APP_HR_ADMIN_STATUS, NULLIF(@@APP_HR_ADMIN_DATE, ''), @@REMARK_1, @@REMARK_2, @@CANCEL_FLAG, @@CANCEL_BY,
					  NULLIF(@@CANCEL_DATE, ''), NULLIF(@@CREATED_DT, ''), @@CREATED_BY, NULLIF(@@UPDATED_DT, ''), @@UPDATED_BY 
					) ;
