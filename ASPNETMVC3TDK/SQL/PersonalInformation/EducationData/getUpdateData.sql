...
DECLARE @@TRANSACTION_ID NVARCHAR(MAX) = @P_TRANSACTION_ID; 
...
DECLARE @@PK_EDUCATION_CD VARCHAR(MAX) = @P_PK_EDUCATION_CD;
DECLARE @@PK_MAJOR_CD VARCHAR(MAX) = @P_PK_MAJOR_CD;  
DECLARE @@COUNT_MATCH2 VARCHAR(MAX);
...
BEGIN TRY	 
	BEGIN 
		...
		-- INSERT FOR UPDATE NEW REDQUEST SUBMISSION
		...
		BEGIN 
			...
			-- THIS SCRIPT FOR NEW LOOP TRANSACTION_ID AS LIKE DATE PROCCESS 
	...
	--		 CHECK ALL APPROVER 4
			...
			SET @@COUNT_MATCH = (
				SELECT COUNT(*) FROM  PDI.TB_M_PROFILE_EDUCATION B 
				WHERE 
				B.NOREG=@@NOREG AND 
				B.EDUCATION_CD=@@EDUCATION_CD AND
				B.MAJOR_CD=@@MAJOR_CD  
			);
			...
			BEGIN  
				...
				IF(@@COUNT_MATCH > 0 AND @@EDUCATION_CD != @@PK_EDUCATION_CD AND @@MAJOR_CD != @@PK_MAJOR_CD  ) 
				BEGIN 
					--
					SET @@info = 'failed';
					SET @@result = 500;
				...
				BEGIN   
					...
					IF(@@CERTIFICATE_NAME IS NULL AND @@CERTIFICATE_PATH IS NOT NULL) 
						BEGIN -- this condition is null from model setup

							UPDATE PDI.TB_M_PROFILE_EDUCATION 
							SET  
								MAJOR_CD = @@MAJOR_CD,
								VALID_FROM = @@VALID_FROM, 
								VALID_TO = @@VALID_TO, GPA = @@GPA, SCHOOL_NAME = @@SCHOOL_NAME, COUNTRY_ID = @@COUNTRY_ID, REMARK_1 = @@REMARK_1, 
								UPDATED_DT = GETDATE(), UPDATED_BY = @@NOREG 
							WHERE 
								NOREG = @@NOREG AND EDUCATION_CD = @@PK_EDUCATION_CD AND MAJOR_CD = @@PK_MAJOR_CD ; 
						...
						BEGIN
							...
							SET 
								...
								, UPDATED_DT = GETDATE()
								, UPDATED_BY = @@NOREG 
							WHERE NOREG = @@NOREG AND EDUCATION_CD = @@PK_EDUCATION_CD AND MAJOR_CD = @@PK_MAJOR_CD ;
				 
						END;
						
					
					INSERT INTO PDI.TB_R_SUBMISSION_EDUCATION
					(TRANSACTION_ID, NOREG, EDUCATION_CD, MAJOR_CD, VALID_FROM, VALID_TO
				, GPA
				, SCHOOL_NAME
				, CERTIFICATE_NAME
				, CERTIFICATE_PATH
			...
			(  
				...
				, ''+LTRIM(RTRIM(@@NOREG))+'' 
				, ''+LTRIM(RTRIM(@@EDUCATION_CD))+''  
				, LTRIM(RTRIM(@@MAJOR_CD)) 
				, @@VALID_FROM  
				, @@VALID_TO
				, @@GPA 
				, @@SCHOOL_NAME
