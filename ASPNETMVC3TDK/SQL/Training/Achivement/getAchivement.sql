...
FROM (
    SELECT
        ...
        IIF(B.TRANSACTION_ID IS NULL, A.SEQ, B.SEQ) AS SEQ,
        IIF(B.TRANSACTION_ID IS NULL, A.TRAINING_TYPE, B.TRAINING_TYPE) AS TRAINING_TYPE,
        IIF(B.TRANSACTION_ID IS NULL, A.INSTITUTION, B.INSTITUTION) AS INSTITUTION,
        IIF(B.TRANSACTION_ID IS NULL, A.COUNTRY_ID, B.COUNTRY_ID) AS COUNTRY_ID,
        IIF(B.TRANSACTION_ID IS NULL, A.YEAR, B.YEAR) AS YEAR,
        IIF(B.TRANSACTION_ID IS NULL, A.TRAINING_TOPIC, B.TRAINING_TOPIC) AS TRAINING_TOPIC,
        IIF(B.TRANSACTION_ID IS NULL, A.SKILL, B.SKILL) AS SKILL,
    ...
    LEFT JOIN PDI.TB_R_SUBMISSION_TRAINING_HISTORY B ON A.NOREG = B.NOREG AND A.SEQ = B.SEQ AND A.TRAINING_TYPE = B.TRAINING_TYPE 
        AND B.CANCEL_FLAG IS NULL AND B.STATUS_CD != ''99''
    WHERE A.NOREG = '''+@@NOREG+'''
    UNION
    SELECT
		B.TRANSACTION_ID,
        ...
        B.CANCEL_BY,
        B.CANCEL_DATE
    FROM PDI.TB_R_SUBMISSION_TRAINING_HISTORY B
    WHERE B.NOREG = '''+@@NOREG+'''
    AND B.CANCEL_FLAG IS NULL
    AND B.STATUS_CD != 99
) SQ
LEFT JOIN DBO.TB_M_COUNTRY C ON SQ.COUNTRY_ID=C.COUNTRY_CD
...
IF(@@REQ_STATUS != '')  BEGIN
    SET @@QUERY = @@QUERY + ' AND SQ.NOREG = '''+@@NOREG+'''  
	 and SQ.TRAINING_TYPE = 3
     ORDER BY SQ.YEAR DESC
 ';
END;
